...
export class SeatBookingService {
  journey: any;
  buses: IBus[] = (data as any).default;
  ...
  confirmBooking() {
    this.updateBusInfo();
    return this.ticketGenerateService.generateTicket(
      this.journey,
      this.getBusById(this.journey.busId)
    );
  }

  updateBusInfo() {
    const busIndex = this.buses.findIndex(
      (element) => element.id == this.journey.busId
    );
    if (this.buses[busIndex].bookings[this.journey.departureDate])
      var bookedStatus = [
        ...this.buses[busIndex].bookings[this.journey.departureDate],
      ];
    else var bookedStatus = new Array(36).fill(false);
